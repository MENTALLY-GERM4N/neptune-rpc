...
import type { ExtendedMediaItem } from "./Caches/ExtendedTrackItem";
...
const includesLower = (string: string, includes: string) =>
	string.toLowerCase().includes(includes.toLowerCase());

export const fullTitle = (
	tidal: MediaItem | MediaItem["album"],
	musicBrainz?: ITrack | IReleaseMatch,
) => {
	...
	if (tidalTitle?.includes("feat. ") && !brainzTitle?.includes("feat. ")) {
		...
		const mbHasFeat =
			artistCredit &&
			artistCredit.findIndex((credit) => credit.joinphrase === " feat. ") !==
				-1;
...
const formatArtists = (
	artists?: (string | undefined)[] | Album["artists"] | TrackItem["artists"],
): string[] =>
	artists
		?.flatMap((artist) => {
...
const resolveArtist = (trackItem: MediaItem, album?: Album) => {
	const sharedAlbumArtist = trackItem.artists?.find(
		(artist) => artist?.id === album?.artist?.id,
	);
	if (sharedAlbumArtist?.name !== undefined)
		return formatArtists([sharedAlbumArtist.name]);
	if ((trackItem.artists?.length ?? -1) > 0)
		return formatArtists(trackItem.artists);
	if (trackItem.artist?.name !== undefined)
		return formatArtists([trackItem.artist.name]);
};
...
export const makeTags = async (
	extTrackItem: ExtendedMediaItem,
): Promise<MetaTags> => {
	const lyrics = interceptPromise(
		() =>
			actions.content.loadItemLyrics({
				itemId: extTrackItem.tidalTrack.id!,
				itemType: "track",
			}),
		...
		.catch(() => undefined)
		.then((res) => res?.[0]);
	const { tidalTrack, releaseAlbum, releaseTrack, tidalAlbum } =
		await extTrackItem.everything();

	const tags: FlacTags = {};
	if (tidalTrack.title !== undefined)
		tags.title = fullTitle(tidalTrack, releaseTrack);

	if (tidalTrack.trackNumber !== undefined)
		tags.trackNumber = tidalTrack.trackNumber.toString();
	...
	if (tidalTrack.contentType === "track") {
		...
		if (tidalTrack.replayGain)
			tags.REPLAYGAIN_TRACK_GAIN = tidalTrack.replayGain.toString();
	}
	...
	if (tidalAlbum !== undefined) {
		if ((tidalAlbum.artists?.length ?? -1) > 0)
			tags.albumArtist = formatArtists(tidalAlbum.artists);
		else if (tidalAlbum.artist?.name)
			tags.albumArtist = [tidalAlbum.artist.name];

		if (tidalAlbum.genre) tags.genres = tidalAlbum.genre;
		if (tidalAlbum.recordLabel) tags.organization = tidalAlbum.recordLabel;
		if (tidalAlbum.numberOfTracks)
			tags.totalTracks = tidalAlbum.numberOfTracks.toString();
		if (!tags.date && tidalAlbum.releaseDate)
			tags.date = tidalAlbum.releaseDate;
		if (!tags.date && tidalAlbum.releaseYear)
			tags.date = tidalAlbum.releaseYear.toString();
