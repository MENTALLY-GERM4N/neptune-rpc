...
export class AlbumCache {
	...
	private static readonly _trackItemsCache: SharedObjectStore<
		ItemId,
		{ albumId: ItemId; trackItems?: TrackItem[] }
	> = new SharedObjectStore("AlbumCache.trackItems", { keyPath: "albumId" });
	public static async get(albumId?: number) {
		if (albumId === undefined) return undefined;

		const album = AlbumCache._cache[albumId];
		if (album !== undefined) return album;

		const albums: Record<number, Album> = store.getState().content.albums;
		for (const albumId in albums) AlbumCache._cache[albumId] = albums[albumId];

		if (AlbumCache._cache[albumId] === undefined) {
			const album = await interceptPromise(
			...
			if (album !== undefined) AlbumCache._cache[albumId] = album;
		}

		return AlbumCache._cache[albumId];
	}
	public static async getTrackItems(playlistUUID?: ItemId) {
		if (playlistUUID === undefined) return undefined;

		const albumTrackItems = await AlbumCache._trackItemsCache.get(playlistUUID);
		const updatePromise = AlbumCache.updateTrackItems(playlistUUID);
		if (albumTrackItems?.trackItems !== undefined)
	...
	public static async updateTrackItems(albumId: ItemId) {
		const result = await interceptPromise(
			...
			libTrace.warn.withContext("PlaylistCache.getTrackItems.interceptPromise"),
		);
		if (result?.[0]?.mediaItems === undefined) {
			const albumTrackItems = await AlbumCache._trackItemsCache.get(albumId);
			return albumTrackItems?.trackItems;
		}
		const trackItems = Array.from(
			(<Immutable.List<MediaItem>>result?.[0]?.mediaItems)
				.map((mediaItem) => mediaItem?.item)
				.filter((item) => item?.contentType === "track"),
		);
		await AlbumCache._trackItemsCache.put({ albumId, trackItems });
		return trackItems;
	}
}
