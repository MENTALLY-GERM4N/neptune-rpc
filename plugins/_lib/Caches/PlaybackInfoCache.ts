import { audioQualities, type AudioQuality } from "../AudioQualityTypes";
...
import {
	...
	ManifestMimeType,
	type TidalManifest,
} from "./PlaybackInfoTypes";

const getCredentials = findModuleFunction<
	() => Promise<{ token: string; clientId: string }>
>("getCredentials", "function");
if (getCredentials === undefined)
	throw new Error("getCredentials method not found");

export class PlaybackInfoCache {
	private static readonly _store: SharedObjectStoreExpirable<
		[
			ExtendedPlayackInfo["playbackInfo"]["trackId"],
			ExtendedPlayackInfo["playbackInfo"]["audioQuality"],
		],
		ExtendedPlayackInfo
	...
	): Promise<ExtendedPlayackInfo> {
		if (!audioQualities.includes(audioQuality))
			throw new Error(
				`Cannot get Stream Info! Invalid audio quality: ${audioQuality}, should be one of ${audioQualities.join(", ")}`,
			);
		if (trackId === undefined)
			throw new Error("Cannot get Stream Info! trackId is missing");

		const playbackInfo = await PlaybackInfoCache._store.get([
			trackId,
			audioQuality,
		]);
		if (playbackInfo !== undefined) return playbackInfo;
		return PlaybackInfoCache.update(trackId, audioQuality);
	}
	static async update(
		trackId: number,
		audioQuality: AudioQuality,
	): Promise<ExtendedPlayackInfo> {
		const release = await PlaybackInfoCache._sema.obtain();
		try {
			...
			switch (playbackInfo.manifestMimeType) {
				case ManifestMimeType.Tidal: {
					const manifest: TidalManifest = JSON.parse(
						atob(playbackInfo.manifest),
					);
					const extPlaybackInfo: ExtendedPlayackInfo = {
						playbackInfo,
					...
					if (expires !== null)
						PlaybackInfoCache._store.putExpires(
							extPlaybackInfo,
							+expires * 1000,
						);
				...
				case ManifestMimeType.Dash: {
					const manifest = await parseDasha(
						atob(playbackInfo.manifest),
						"https://sp-ad-cf.audio.tidal.com",
					);
					return {
						playbackInfo,
						manifestMimeType: playbackInfo.manifestMimeType,
				...
				default: {
					throw new Error(
						`Unsupported Stream Info manifest mime type: ${playbackInfo.manifestMimeType}`,
					);
				}
			}
		} catch (e) {
