...
export const requestJsonCached: NativeBridge["requestJsonCached"] = async (
	url,
	options,
) => {
	...
	const liveCacheRes = _requestJsonCached(url, options).then((apiRes) => {
		requestCache
			.put(apiRes, url)
			.catch(libTrace.err.withContext("requestCache.put"));
		return apiRes;
	});
	const cacheRes = await requestCache
		.get(url)
		.catch(libTrace.err.withContext("requestCache.get"));
