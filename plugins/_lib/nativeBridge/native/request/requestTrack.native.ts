...
export type ExtendedPlaybackInfoWithBytes = ExtendedPlayackInfo & {
	stream: Readable;
};
export const requestTrackStream = async (
	{ manifestMimeType, manifest }: ExtendedPlayackInfo,
	fetchyOptions: FetchyOptions = {},
): Promise<Readable> => {
	switch (manifestMimeType) {
		case ManifestMimeType.Tidal: {
			return requestDecodedStream(manifest.urls[0], {
				...fetchyOptions,
				manifest,
			});
		}
		...
		default: {
			throw new Error(
				`Unsupported Stream Info manifest mime type: ${manifestMimeType}`,
			);
		}
	}
};
