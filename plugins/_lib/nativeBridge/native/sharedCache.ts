...
export const cacheEnsure = async <V>(
	key: string | number | symbol,
	cacheId = "_",
): Promise<[true, V] | [false]> => {
	if (caches[cacheId]?.[key]?.promise !== undefined)
		return [true, await caches[cacheId][key].promise];
	...
	caches[cacheId][key].promise = new Promise<V>((_res, _rej) => {
		...
		setTimeout(
			() =>
				_rej(
					new Error("Timeout! Promise did not get resolved within 1 minute."),
				),
			60 * 1000,
		);
	});
