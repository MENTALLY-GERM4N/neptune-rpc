				...
				if (coverUrl !== undefined) {
					try {
						picture = {
							...
							buffer: await requestStream(coverUrl)
								.then(rejectNotOk)
								.then(toBuffer),
						};
					} catch {}
...
export const startTrackDownload = async (
	extPlaybackInfo: ExtendedPlayackInfo,
	pathInfo: PathInfo,
	metaTags?: MetaTags,
): Promise<void> => {
	const pathKey = JSON.stringify(pathInfo);
	if (downloadStatus[pathKey] !== undefined)
		throw new Error(`Something is already downloading to ${pathKey}`);
	try {
		...
		const stream = await requestTrackStream(extPlaybackInfo, {
			onProgress: (progress) => (downloadStatus[pathKey] = progress),
		});
		const metaStream = await addTags(extPlaybackInfo, stream, metaTags);
		const writeStream = createWriteStream(
			`${folderPath}${pathSeparator}${pathInfo.fileName}`,
		);
...
export const getDownloadProgress = (filePath: string) =>
