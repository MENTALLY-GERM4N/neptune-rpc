...
import { type MetaTags, makeTags } from "./makeTags";
...
export const isStartingPlaying = () => {
	...
	return (
		isPlaying(desiredPlaybackState) && playbackState !== desiredPlaybackState
	);
};
export const isPlaying = (desiredPlaybackState?: PlaybackState) => {
	desiredPlaybackState ??=
		store.getState().playbackControls.desiredPlaybackState;
...
export const getCurrentTrack = async (
	...
	if (playbackContext === undefined)
		throw new Error("PlaybackContext is undefined");
...
export const registerOnScrobble = ({
	onScrobble,
	onNowPlaying,
}: {
	onScrobble?: (currenTrack: CurrentTrack) => void;
	onNowPlaying?: (currentTrack: CurrentTrack) => void;
}) => {
	let currentTrack: CurrentTrack | undefined = undefined;
	const updateNowPlaying = debounce(
		async (playbackContext?: PlaybackContext) => {
			if (!isPlaying()) return;
			currentTrack = await getCurrentTrack(playbackContext).catch(
				trace.msg.err.withContext("Failed to get current track!"),
			);
				...
				switch (state) {
					...
					default: {
						if (lastPlayStart !== null)
							totalPlayTime += Date.now() - lastPlayStart;
						lastPlayStart = null;
					}
				}
			}),
				...
				intercept(
					...
					([{ playbackContext }]) => {
						if (currentTrack !== undefined) {
							...
							const minPlayTime =
								+currentTrack.playbackContext.actualDuration *
								MIN_SCROBBLE_PERCENTAGE *
								1000;
